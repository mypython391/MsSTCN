...
class Trainer:
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            print(f'Train MAE: {np.array(train_mae).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)
            print(f'Train RMSE: {np.array(train_rmse).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)

            val_loss, val_mae, val_rmse = self.validate()
            print(f'Val Loss: {val_loss:.4f}')
            print(f'Val MAE: {np.array(val_mae).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)
            print(f'Val RMSE: {np.array(val_rmse).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)
    ...
    def fit(self):
        ...
        counter = 0
        running_loss = 0.
        running_mae = [0.] * 12  # 恢复：12个时间步
        running_rmse = [0.] * 12  # 恢复：12个时间步
        ...
        for i, data in prog_bar:
            ...
            # 使用加权损失函数（需要完整张量）
            if hasattr(self.model, 'weighted_mse_loss'):
                # 处理NaN值
                idx = ~torch.isnan(y_true)
                y_pred_clean = y_pred.clone()
                y_true_clean = y_true.clone()
                y_pred_clean[~idx] = 0
                y_true_clean[~idx] = 0
                loss = self.criterion(y_pred_clean, y_true_clean)
            else:
                # 标准损失函数
                idx = ~torch.isnan(y_true)
    ...
    def report_test_error(self):
        ...
        print(f'Test MAE: {np.array(test_mae).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)
        print(f'Test RMSE: {np.array(test_rmse).reshape(2,6)}')  # 恢复：12个时间步reshape为(2,6)

        return test_loss, test_mae, test_rmse

    def rescale_output(self, y_true, y_pred):
        for i in range(12):  # 恢复：固定12个时间步
